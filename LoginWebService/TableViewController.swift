...
class TableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        AF.request(urlStr, method: .get).validate().responseJSON{ response in
            if let arreglo = response.value as?  [[String: String]] {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Primero obtener 
        let elDict = dataSource[indexPath.row]
        
        cell.textLabel?.text = (elDict["title"] ?? "")
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        // Obetener el indexPath de la celda seleccionada
        if let indexPath = tableView.indexPathForSelectedRow {
            let elDict = dataSource[indexPath.row]
            
            let destino = segue.destination as! ViewControllerPhoto
            destino.nombreFoto = (elDict["pict"] ?? "")
        }
